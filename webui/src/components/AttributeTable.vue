...
function explanation(row) {
  if (row.status === 'RED') return 'Below threshold'
  if (row.status === 'YELLOW') return 'Approaching threshold'
  return 'Normal'
}
</script>
      ...
      <table class="w-full border-collapse text-sm">
        ...
        <tbody>
          ...
          >
            ...
            <td class="mono px-4 py-2.5 text-right text-[var(--text-secondary)]">{{ row.value }}</td>
